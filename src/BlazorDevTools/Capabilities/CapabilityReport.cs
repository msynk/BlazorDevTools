...
internal static class CapabilityReport
{
    ...
    public static IReadOnlyList<DevToolsCapability> Build(DevToolsSession session)
    {
        ...
        var diffAttributionLost = session.RenderTracker.DiffAttributionLost;
        ...
        [
            ...
            new("Rendering", "Batch diff duration and DOM edit count", diffAttributionLost ? CapabilityStatus.NotReliable : batchMetricsObserved ? CapabilityStatus.Available : metricsSupported ? CapabilityStatus.RequiresRuntimeSupport : CapabilityStatus.NotAvailable, diffAttributionLost ? "Attribution stopped after the browser acknowledgement backlog exceeded the bounded queue. Later measurements are omitted rather than attached to the wrong batch." : batchMetricsObserved ? "Observed through the aspnetcore.components.render_diff meters (.NET 10). On Blazor Server the measurement arrives when the browser acknowledges the render, so it is matched back to the batch it belongs to." : metricsHint),
